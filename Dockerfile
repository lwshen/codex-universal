...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    ...
    --mount=type=cache,target=$HOME/.cache/pipx \
    apt-get update \
...
RUN --mount=type=cache,target=$HOME/.npm \
    ...
    --mount=type=cache,target=$HOME/.local/share/pnpm/store \
...
RUN --mount=type=cache,target=$HOME/.cache/mise \
...
ENV SWIFTLY_BIN_DIR=$HOME/.swiftly/bin
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,target=/var/lib/apt,sharing=locked \
    --mount=type=cache,target=$HOME/.cache/mise \
    apt-get update && apt-get install -y --no-install-recommends \
    libyaml-dev=0.2.* \
    libgmp-dev=2:6.3.* \
    && rm -rf /var/lib/apt/lists/* \
...
RUN set -eux; \
    groupadd codex; \
    useradd --create-home --home-dir /home/codex --shell /bin/bash --gid codex codex; \
