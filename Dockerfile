...
RUN set -eux; \
    existing_group_by_name=$(getent group "$USERNAME" || true); \
    existing_group_by_gid=$(getent group "$USER_GID" || true); \
    if [ -n "$existing_group_by_name" ]; then \
        :; \
    elif [ -n "$existing_group_by_gid" ]; then \
        groupmod -n "$USERNAME" "$(echo "$existing_group_by_gid" | cut -d: -f1)"; \
    else \
    ...
    if id -u "$USERNAME" >/dev/null 2>&1; then \
        ...
        useradd -o --uid "$USER_UID" --gid "$USER_GID" --home-dir /root --shell /bin/bash --no-create-home "$USERNAME"; \
    else \
